...
$text =~ s/LABEL (.+)/\tstop music\n\treturn\n\nlabel $1:/g;
$text =~ s/CHOICE (.+)\nJUMP TO/\t\t"$1":\n\t\t\tjump/g;
$text =~ s/JUMP TO/\tjump/g;
$text =~ s/BEGIN.CHOICES?:/BEGIN CHOICES/g;
$text =~ s/BEGIN.CHOICES?/\tmenu:/g;
$text =~ s/CHOICE:/CHOICE/g;
$text =~ s/CHOICE (.+)/\t\t"$1":/g;
$text =~ s/END.CHOICES?/\n/g;
$text =~ s/STOP MUSIC/\tstop music/g;
$text =~ s/FADEOUT/fadeout/g;
$text =~ s/FADEIN/fadein/g;
